...
export function calculateCost(args) {
    ...
    if (!cost || !args.tokens)
        return undefined;
    const { prompt_tokens: prompt, completion_tokens: completion = 0 } = args.tokens;
    // Work around floating point errors by multiplying by 1000 and rounding.
    const MULTIPLIER = 1000;
    const multipliedCost = cost.input * prompt * MULTIPLIER + cost.output * completion * MULTIPLIER;
...
function getCost(model) {
    ...
    if (model.startsWith('babbage-002'))
        return COSTS['babbage-002'];
    if (model.startsWith('davinci-002'))
        return COSTS['davinci-002'];
    ...
    if (model.startsWith('ft:davinci'))
        return COSTS['ft:davinci-002'];
    if (model.includes('ft:babbage'))
        return COSTS['ft:babbage-002'];
    if (model.includes('ft:gpt-3.5-turbo'))
        return COSTS['ft:gpt-3.5-turbo'];
    // V1 fine-tuned models
    // Eg: ada:ft-your-org:custom-model-name-2022-02-15-04-21-04
    if (model.startsWith('ada:ft'))
        return COSTS['ft-legacy:ada'];
    if (model.startsWith('babbage:ft'))
        return COSTS['ft-legacy:babbage'];
    return null;
}
