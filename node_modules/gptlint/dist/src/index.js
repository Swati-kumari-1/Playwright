...
function mergeEvalStats(evalStatsA, evalStatsB) {
  return {
    numFiles: evalStatsA.numFiles + evalStatsB.numFiles,
    numRules: evalStatsA.numRules + evalStatsB.numRules,
    numUnexpectedErrors: evalStatsA.numUnexpectedErrors + evalStatsB.numUnexpectedErrors,
    numFalseNegatives: evalStatsA.numFalseNegatives + evalStatsB.numFalseNegatives,
    numFalsePositives: evalStatsA.numFalsePositives + evalStatsB.numFalsePositives,
...
var knownSupportedCodeFileExtensions = /* @__PURE__ */ new Set([
  "js",
  "ts",
  "mjs",
  "cjs",
...
function inferBestPossibleCodeFileExtension(lang, { fallbacks } = {}) {
  function handleFallbacks() {
    if (fallbacks) {
      for (const fallback of fallbacks) {
        const alias2 = inferBestPossibleCodeFileExtension(fallback);
        if (alias2) return alias2;
      }
    }
    return void 0;
  }
  lang = lang == null ? void 0 : lang.trim().toLowerCase();
...
function logEvalStats({
  evalStats
}) {
  const precision = evalStats.numTruePositives / (evalStats.numTruePositives + evalStats.numFalsePositives);
  const recall = evalStats.numTruePositives / (evalStats.numTruePositives + evalStats.numFalseNegatives);
...
var defaultCacheDir = findCacheDirectory({ name: "gptlint" }) ?? path2.join(".gptlint", "cache");
...
function parseRuleNode({
  ...
  const fileNameRuleName = path4.basename(filePath).replace(/\.\w+$/, "");
...
async function parseRuleFilePath(filePath, {
  cwd = process.cwd()
} = {}) {
  const filePathResolved = cwd ? path5.resolve(cwd, filePath) : filePath;
  const content = await fs2.readFile(filePathResolved, { encoding: "utf8" });
...
async function resolveEvalFiles({
  ...
  const sourceFiles = await resolveGlobFilePatterns(config.files, {
    gitignore: false,
    ignore: config.ignores.filter((pattern) => !/fixtures/.test(pattern)).concat(["node_modules", "dist", "docs", ".env", ".next"]),
    cwd
  });
...
export {
  LLMOptionsSchema,
  LinterConfigOverrideSchema,
  LinterConfigOverridesSchema,
  LinterConfigRuleSettingSchema,
  LinterConfigRuleSettingsSchema,
  LinterConfigSchema,
  LinterOptionsSchema,
  ResolvedLinterConfig,
  RuleDefinitionExampleSchema,
  ...
  logLintResultStats,
  mergeEvalStats,
  mergeLinterConfigRuleSettings,
  mergeLinterConfigs,
  mergeLinterConfigsOverride,
  omit,
  parseLinterConfig,
  parseRuleFile,
  parseRuleFilePath,
